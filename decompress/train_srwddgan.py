...
def train(rank, gpu, args):
    ...
    # parent_dir = os.path.join(current_dir, parent_dir)
    ...
    # 0.8;0.2 for celebA
    # 0.9;0.1 for df2k
    train_size = int(0.50 * len(dataset))  # in DY, 0.5 means 1 year and 0.25 means 6 months
    test_size = len(dataset) - train_size  

    # Set a seed for reproducibility
    # random split of the dataset
    torch.manual_seed(42) 
    # Note that we should not shuffle the dataset to keep the order of the images!
    #train_set, test_set = torch.utils.data.random_split(dataset, [train_size, test_size])

    # keep the order of the images
    # Create indices for the split (without shuffling)
